...
select_base_genome <- function(output_matrix_file) {
  ...
  temp <- read.table(output_matrix_file,header = TRUE)
  ...
  for (i in 1:length(base_genome_names)) {
    av_count <- sum(str_count(as.matrix(temp[,longest_base_genomes]),as.character(fixed(base_genome_names[i]))),na.rm=TRUE)/(dim(temp)[1]*length(taxa_names))
